...
16. **Отсутствие единой дизайн-системы** ✅ **РЕШЕНО**
   - ~~Каждая страница выглядит по-своему~~ → Создан `design-system.css` с CSS переменными
   - ~~Разные отступы, шрифты, цвета~~ → Унифицированы через CSS tokens
   - ~~Нет CSS переменных для цветовой схемы~~ → Добавлены переменные для светлой/темной темы
   - **Решение:** Создан `public/css/design-system.css` с токенами, подключен ко всем 20+ страницам17. **Дублирование кода работы с API** ✅ **РЕШЕНО**
   - ~~Каждая страница делает свои fetch запросы~~ → Создан централизованный API клиент
   - ~~Повторяющаяся логика обработки ошибок~~ → Унифицирована в `API.request()`
   - ~~Нет централизованного API клиента~~ → Создан `public/js/utils/api.js`
   - **Решение:** API клиент с методами для всех ресурсов, timeout 12s, нормализация ошибок18. **Плохая адаптивность** ✅ **ЧАСТИЧНО РЕШЕНО**
    - ~~Многие страницы плохо работают на мобильных~~ → Добавлены responsive стили для таблиц
    - ~~Нет mobile-first подхода~~ → Создан `responsive-tables.css` с mobile-first
    - ~~Сложные таблицы не адаптируются~~ → Таблицы преобразуются в card view на мобильных
    - **Решение:** Responsive стили для categories/accounts/transactions
